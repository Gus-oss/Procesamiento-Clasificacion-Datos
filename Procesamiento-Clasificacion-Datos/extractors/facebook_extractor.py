...
class FacebookExtractor:
    ...
    # -----------------------------------------
    # 1. Extraer posts de una pagina
    # -----------------------------------------
    def get_posts(self, page_url, max_posts=5):
        ...
        print(f"   {len(posts)} posts extraidos")
        return posts

    # -----------------------------------------
    # 2. Extraer comentarios de los posts
    # -----------------------------------------
    def get_comments(self, post_urls, max_comments=50):
        ...
        print(f"   {len(comments)} comentarios extraidos")
        return comments
    ...
    def _wait_and_fetch(self, run_id, timeout=600):
        ...
        while elapsed < timeout:
            ...
            if state == "SUCCEEDED":
                ...
                print(f"   Run fallo con estado: {state}")
    ...
    # 4. Pipeline completo
    # -----------------------------------------
    def run_pipeline(self, page_url, max_posts=3, max_comments=50):
        ...
        print(f"\nColumnas en posts: {df_posts.columns.tolist()}")
        print(f"Claves del primer post: {list(posts[0].keys())}\n")
        ...
        if not post_urls:
            ...
            print(f"   Claves disponibles: {list(posts[0].keys())}")
